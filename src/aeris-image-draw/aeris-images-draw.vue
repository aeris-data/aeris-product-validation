...
export default {
  ...
  methods: {
      ...
      return {
        selection: function(event) {
          ...
          var chart = this,
            ...
            vm.id = vm.idPlot+ vm.num++;
            vm.$set(vm.parameterPlotband, vm.id, [vm.blankPlotband]);
          ...
          response => {
            ...
            _this.chart = Highcharts.chart("graph", _this.options, function() {
              ...
              $.each(this.series, function(j, data) {
                $legend.append(
                  '<div class="item"><div class="symbol" style="background-color:' +
                    data.color +
      ...
      if (Object.values(this.uuidArr).indexOf(this.uuid) > -1) {
        ...
        if (this.canvas) {
          ...
          window.setTimeout(function() {
            ...
            _this.viewportTransform =
              _this.canvasTab[canvasName].viewportTransform;
          }, 5);
    ...
    resizeCanvas: debounce(function(canvas) {
      ...
      $(img).on(
        ...
        function() {
          ...
          if (img.width > 800) {
            ...
            $(".aeris-images-draw-relative-container").css(
              "min-height",
              img.height
            );
            $(".aeris-images-draw-relative-container").css(
        ...
        containers.forEach(function (el) {
          ...
          canvas.setZoom(1);
          this.bgImg = canvas.setBackgroundImage(
            img.src,
            function() {
      ...
      this.valueYmax = this.convert2unitY(
        this.computeY(this.pixelsBounds.tl.y, canvas),
        canvas
      );
      ...
      data.elements.forEach(
        function(elem) {
          var pointer = {
            ...
            y: this.computeY(
              this.convert2pixelY(elem.coords.start.y, canvas),
              canvas
            )
          };
          ...
          var pixels = {
            ...
            stop: {
              ...
              y: this.computeY(
                this.convert2pixelY(elem.coords.stop.y, canvas),
                canvas
              )
            }
          };
    ...
    handleMouseMove: debounce(function(ev) {
      ...
      if (this.mousedown && this.isDrawing) {
        ...
        if (this.elementType === "zone") {
          this.rect.set({
            height: Math.abs(_this.origY - this.mousePosition.y)
          });
        ...
        if (elt != null) {
          if (elt.aeris.comment != "" || elt.aeris.quality != "") {
            $("#tooltip").css({
              left: Math.abs(ev.e.offsetX) + 25,
              top: Math.abs(ev.e.offsetY) + 25
            });
    ...
    handleMouseWheel: function(ev) {
      ...
      if (delta) {
        ...
        this.zoomTo(
          new fabric.Point(this.mousePosition.x, this.mousePosition.y),
          canvas
        );
      }
    },
    ...
    handleObjectselected: function(ev) {
      ...
      if (
        this.selectedElement.id != this.lastSelectedElementId &&
        this.lastSelectedElementId != ""
      ) {
        console.log("current tool tip : " + this.toolTipComment);
        console.log("getDrawElementById Last : " + this.lastSelectedElementId);

        this.getDrawElementById(
          this.lastSelectedElementId
        ).aeris.comment = this.toolTipComment;
        this.getDrawElementById(
          this.lastSelectedElementId
        ).aeris.quality = this.toolTipQuality;
        this.toolTipComment = "";
        this.toolTipQuality = "";
      }
    ...
    handleObjectModified: debounce(function(ev) {
      ...
      (obj.comment = el.aeris.comment), (obj.quality = el.aeris.quality);
    ...
    setRules: function() {
      ...
      var yMin = this.convert2unitY(
        this.computeY(this.pixelsBounds.bl.y, canvas),
        canvas
      );
      var yMax = this.convert2unitY(
        this.computeY(this.pixelsBounds.tl.y, canvas),
        canvas
      );
    ...
    saveCurrentCanvas: function() {
      ...
      canvas.forEachObject(function(obj) {
        ...
        var index = _this.manualElments[canvasId].findIndex(
          i => i.id === obj.id
        );
    ...
    drawEntry: function(entryElt) {
      ...
      var pointer = {
        ...
        y: this.computeY(
          this.convert2pixelY(entryElt.coords.start.y, canvas),
          canvas
        )
      };
      _this.origX = pointer.x;
